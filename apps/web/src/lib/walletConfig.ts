...
// VITE_LPDOCTOR_E2E_MOCK_WALLET=1 at build time), the injected connector
...
function isMockWalletRequested(): boolean {
  if (typeof window !== "undefined") {
    ...
    try {
      if (window.localStorage.getItem("lpdoctor.mockWallet") === "1") {
        return true;
      }
    } catch {
  ...
  return import.meta.env.VITE_LPDOCTOR_E2E_MOCK_WALLET === "1";
}
...
if (mockEnabled && typeof window !== "undefined") {
  try {
    window.localStorage.setItem("lpdoctor.mockWallet", "1");
  } catch {
